...
### 2026-06-20
- **解讀模型 Sonnet 4.6 → Opus 4.8**(`6a82acb`)。對打評估(3 案例):引擎算好的事實兩者全對且一致;但「AI 自推」項(元神、世應生剋方向)**Sonnet 3 案 2 錯、Opus 0 錯**(如國考官鬼元神:Sonnet 誤判父母、Opus 正確妻財),Opus 文筆也較簡潔。成本約 +50~60%/篇(熱快取 $0.040→$0.061、冷 $0.087→$0.142;每篇仍幾毛美金)。`_AI_READING_MODEL` 預設改 opus-4-8(env `AI_READING_MODEL` 可覆寫)。
- **元神 BUG 修正**(`e672a27`,prompt v1.19):batch12 讓 AI 用六親生剋鏈自推元神/忌神,兩 model 都把五行生剋與六親搞混(元神幾乎全錯)→ 改**固定查找表**(用神→元/忌/仇神),兩 model 皆修正。
- **教訓 7**:回歸測試(`validate_classics.py` 5376 卦)只驗「引擎確定性欄位」,**測不到「AI 自推」的錯**(元神 BUG 在 AI 推理層,且加在驗證之後)。準確度風險不是 model,是 prompt 殘留的「叫 AI 自算」點 → 治本是搬進引擎/查表。**待辦:§四 世應生剋方向仍 AI 自推(Sonnet 會錯方向),可搬引擎。**
