...
# 建除十二神的標準宜/忌(中性常用語)
_JIANCHU_YIJI = {
    "建": (["出行", "求財", "見貴"], ["動土", "嫁娶", "安葬", "開倉"]),
    "除": (["祭祀", "掃除", "療病", "解除", "出行"], ["嫁娶", "開市"]),
    "滿": (["祭祀", "納財"], ["嫁娶", "就醫", "動土", "安葬"]),
    "平": (["祭祀"], ["嫁娶", "開市", "出行", "動土", "訴訟"]),
    "定": (["嫁娶", "開市", "入學", "上樑", "安床", "安葬"], ["出行", "訴訟"]),
    "執": (["造作", "立券", "捕捉", "納財"], ["出行", "移徙", "開市"]),
    "破": (["破屋", "求醫療病"], ["諸事不宜"]),
    "危": (["祭祀", "安床", "修造"], ["登高", "乘船", "冒險"]),
    "成": (["嫁娶", "開市", "入學", "修造", "出行", "入宅", "安葬"], ["訴訟"]),
    "收": (["納財", "入倉", "捕捉"], ["出行", "安葬", "開市", "嫁娶"]),
    "開": (["祭祀", "開市", "入學", "修造", "出行", "上任"], ["安葬", "動土"]),
    "閉": (["築堤", "埋葬", "塞穴"], ["開市", "出行", "就醫", "嫁娶"]),
}
...
def day_zeri(year, month, day):
    ...
    # 宜/忌分列:建除標準宜忌 + 神煞覆蓋
    yi, ji = list(_JIANCHU_YIJI[jc][0]), list(_JIANCHU_YIJI[jc][1])
    if sf:
        yi, ji = ["祭祀"], ["諸事不宜"]
    elif jc == "破":
        pass  # 建除破已是「破屋/求醫療病」「諸事不宜」
    elif she:
        yi, ji = ["祈福", "修造", "嫁娶", "開市", "出行", "入宅", "安葬"], []
    elif yd or td:
        yi, ji = ["祈福", "修造", "嫁娶", "開市", "出行", "入宅"], []
    elif tz:
        yi, ji = ["祭祀"], ["動土", "興工", "嫁娶", "開市"]
